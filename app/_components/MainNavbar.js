...
export default function MainNavbar() {
  ...
  return (
    <nav className="container mx-auto min-w-full z-40">
      <div className={`fixed top-0 left-0 right-0 px-6 lg:px-24 py-3 z-40 transition-all duration-500 ${isScrolled ? "bg-white/80 backdrop-blur-xl border-b border-line shadow-sm" : "bg-transparent"}`}>
        {/* Desktop */}
          ...
          <div className='grid grid-cols-3 gap-x-3.5 justify-stretch content-center'>
            ...
            <div className='flex items-center justify-self-end'>
              <Link href="/started" className=''>
                <MainButton btn_txt='Get a surcharge audit' className="!text-xs !px-4" />
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className='block lg:hidden'>
          ...
          {burgerOn ? <div className='grid grid-cols-1 auto-rows-auto gap-y-8 py-6 my-4 justify-items-center content-center bg-white border border-line rounded-lg shadow-sm'>
