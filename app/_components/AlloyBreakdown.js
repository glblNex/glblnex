'use client'

import React from "react";
import { motion } from "motion/react";
import { alloyMetals, riskColor, riskLevel } from "../_data/metals";

export default function AlloyBreakdown({ alloy }) {
  const metals = alloyMetals(alloy);

  return (
    <div className="w-full">
      <div className="flex flex-col gap-5">
        {metals.map((m, i) => {
          const exposure = m.pct / 100;
          const volScore = m.volatility;
          const level = riskLevel(volScore);
          const color = riskColor(volScore);
          return (
            <motion.div
              key={m.key}
              initial={{ opacity: 0, y: 12 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.4, delay: 0.06 * i }}
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <span
                    className="grid place-items-center h-8 w-8 rounded-md text-[11px] font-bold text-white shrink-0"
                    style={{ backgroundColor: color }}
                  >
                    {m.meta.symbol}
                  </span>
                  <div className="leading-tight">
                    <p className="text-sm font-medium text-ink">{m.meta.name}</p>
                    <p className="text-xs text-light">{m.pct}% of alloy · ${m.meta.basePrice.toLocaleString()}/t index</p>
                  </div>
                </div>
                <span
                  className="text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap"
                  style={{ color, backgroundColor: `${color}14` }}
                >
                  {Math.round(volScore * 100)}% volatility
                </span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-light mb-1">Composition</p>
                  <div className="h-2 w-full rounded-full bg-line overflow-hidden">
                    <motion.div
                      className="h-full rounded-full bg-highlight"
                      initial={{ width: 0 }}
                      whileInView={{ width: `${Math.round(exposure * 100)}%` }}
                      viewport={{ once: true }}
                      transition={{ duration: 0.8, delay: 0.08 * i }}
                    />
                  </div>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-light mb-1">Price exposure</p>
                  <div className="h-2 w-full rounded-full bg-line overflow-hidden">
                    <motion.div
                      className="h-full rounded-full"
                      style={{ backgroundColor: color }}
                      initial={{ width: 0 }}
                      whileInView={{ width: `${Math.round(exposure * volScore * 100)}%` }}
                      viewport={{ once: true }}
                      transition={{ duration: 0.8, delay: 0.12 + 0.08 * i }}
                    />
                  </div>
                </div>
              </div>
            </motion.div>
          );
        })}
      </div>
    </div>
  );
}
